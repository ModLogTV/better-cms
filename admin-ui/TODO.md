...
- [x] add a confirmation popover for setting the default locale
- [x] the "Search common locales..." select popover content isnt scrollable (bug)
- [x] redesing the add locale dialog: tab switcher at the top (consisting of two cards; the selected one becomes the primary color for border and text) "Use common preset" or "Custom configuration". Use common preset shows the select component with the country presets and flags, custom configuration shows the code and name input fields
- [x] make sure all checkboxes used get a shadcn checkbox component (for example in the "Set as default" option in the add locale dialog) that is styled like the rest of our components

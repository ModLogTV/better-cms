...
type FakeUser = {
	...
	cmsGroups: Array<{ groupId: string; group: { id: string; name: string; permissions: string[] } }>;
};
...
function makePrisma(overrides: {
	...
	return {
		...
		cmsGroup: {
			...
			update: mock(async (args: { where: { id: string }; data: Partial<FakeGroup> }) =>
...
function makeAuth(sessionUser?: { id: string; email: string; name: string } | null) {
...
describe("betterAuthCMSAdapter – verifyRequest", () => {
	test("resolves session and returns user permissions", async () => {
		const prisma = makePrisma({
			user: { findUnique: mock(async () => makeUser({ cmsPermissions: ["cms:translations:read"] })) },
		});
		const adapter = betterAuthCMSAdapter({ auth: makeAuth(), prisma: prisma as never });
			...
			user: {
				findUnique: mock(async () =>
					makeUser({
						...
						cmsGroups: [{ groupId: "g1", group: makeGroup({ permissions: ["cms:pages:write"] }) }],
					}),
				),
			},
		});
	...
	test("deduplicates permissions across direct and groups", async () => {
		const prisma = makePrisma({
			user: {
				findUnique: mock(async () =>
					makeUser({
						cmsPermissions: ["cms:locales:read"],
						cmsGroups: [{ groupId: "g1", group: makeGroup({ permissions: ["cms:locales:read"] }) }],
					}),
				),
			},
		});
		...
		const count = result.permissions.filter((p) => p === "cms:locales:read").length;
		expect(count).toBe(1);
	});

	test("returns unauthorized when no session", async () => {
		const adapter = betterAuthCMSAdapter({ auth: makeAuth(null), prisma: makePrisma() as never });
	...
	test("returns empty permissions when user not found in DB", async () => {
		const prisma = makePrisma({ user: { findUnique: mock(async () => null as never) } });
		const adapter = betterAuthCMSAdapter({ auth: makeAuth(), prisma: prisma as never });
	...
	test("serviceToken accepted via x-internal-token header", async () => {
		...
		const result = await adapter.verifyRequest({ "x-internal-token": "svc-secret" });
...
describe("betterAuthCMSAdapter – upsertAdminUser", () => {
	test("creates user via signUpEmail when user does not exist", async () => {
		...
		const prisma = makePrisma({
			user: {
				findUnique: mock(async () => {
					...
					return callCount === 1 ? (null as never) : makeUser({ cmsPermissions: [] });
				}),
			},
		});
		const auth = makeAuth();
		...
		await adapter.upsertAdminUser!({ email: "admin@example.com", name: "Admin", password: "secret" });
	...
	test("skips signUpEmail when user already exists", async () => {
		const prisma = makePrisma({
			user: { findUnique: mock(async () => makeUser({ cmsPermissions: [CMS_WILDCARD_PERMISSION] })) },
		});
		const auth = makeAuth();
		const adapter = betterAuthCMSAdapter({ auth, prisma: prisma as never });
	...
	test("skips permission update when wildcard already set", async () => {
		...
		const prisma = makePrisma({
			user: {
				findUnique: mock(async () => makeUser({ cmsPermissions: [CMS_WILDCARD_PERMISSION] })),
				update: updateMock,
			},
		});
		const adapter = betterAuthCMSAdapter({ auth: makeAuth(), prisma: prisma as never });
	...
	test("throws when user cannot be found after creation", async () => {
		...
		await expect(
			adapter.upsertAdminUser!({ email: "admin@example.com", name: "Admin", password: "secret" }),
...
describe("betterAuthCMSAdapter – management.listUsers", () => {
	test("maps user records to CMSUserSummary", async () => {
		...
		const users = await adapter.management!.listUsers();
...
describe("betterAuthCMSAdapter – management.getUserPermissions", () => {
	test("resolves merged permissions", async () => {
		...
		const perms = await adapter.management!.getUserPermissions({ userId: "user-1" });
...
describe("betterAuthCMSAdapter – management.setUserPermissions", () => {
	test("calls prisma.user.update with new permissions", async () => {
		...
		await adapter.management!.setUserPermissions({ userId: "user-1", permissions: ["cms:translations:write"] });
...
describe("betterAuthCMSAdapter – management.getUserGroups", () => {
	test("returns groups from user record", async () => {
		const prisma = makePrisma({
			user: {
				findUnique: mock(async () =>
					makeUser({
						cmsGroups: [{ groupId: "g1", group: makeGroup({ id: "g1", name: "Editors", permissions: ["cms:pages:write"] }) }],
					}),
				),
			},
		});
		...
		const groups = await adapter.management!.getUserGroups({ userId: "user-1" });
	...
	test("returns empty array when user not found", async () => {
		...
		const groups = await adapter.management!.getUserGroups({ userId: "missing" });
...
describe("betterAuthCMSAdapter – management group membership", () => {
	test("addUserToGroup calls cmsUserGroup.create with correct data", async () => {
		...
		await adapter.management!.addUserToGroup({ userId: "u1", groupId: "g1" });
	...
	test("removeUserFromGroup calls cmsUserGroup.delete with composite key", async () => {
		...
		await adapter.management!.removeUserFromGroup({ userId: "u1", groupId: "g1" });
...
describe("betterAuthCMSAdapter – management groups CRUD", () => {
	test("listGroups returns mapped groups", async () => {
		const prisma = makePrisma({
			cmsGroup: { findMany: mock(async () => [makeGroup({ id: "g1", name: "Admins", permissions: ["cms:*"] })]) },
		});
		const adapter = betterAuthCMSAdapter({ auth: makeAuth(), prisma: prisma as never });

		const groups = await adapter.management!.listGroups();

		expect(groups).toHaveLength(1);
		expect(groups[0]).toEqual({ id: "g1", name: "Admins", permissions: ["cms:*"] });
	});

	test("createGroup calls cmsGroup.create with correct data", async () => {
		const createMock = mock(async (args: { data: { name: string; permissions: string[] } }) =>
			makeGroup({ name: args.data.name, permissions: args.data.permissions }),
		);
		const prisma = makePrisma({ cmsGroup: { create: createMock } });
		const adapter = betterAuthCMSAdapter({ auth: makeAuth(), prisma: prisma as never });

		const group = await adapter.management!.createGroup({ name: "Editors", permissions: ["cms:translations:write"] });
	...
	test("updateGroup spreads opts excluding id into data", async () => {
		const updateMock = mock(async (args: { where: { id: string }; data: Partial<FakeGroup> }) =>
			makeGroup({ id: args.where.id, ...args.data }),
		);
		const prisma = makePrisma({ cmsGroup: { update: updateMock } });
		const adapter = betterAuthCMSAdapter({ auth: makeAuth(), prisma: prisma as never });

		await adapter.management!.updateGroup({ id: "g1", name: "Updated" });
	...
	test("deleteGroup calls cmsGroup.delete with id", async () => {
		...
		await adapter.management!.deleteGroup({ id: "g1" });

		expect(deleteMock).toHaveBeenCalledTimes(1);
		const args = (deleteMock.mock.calls as unknown[][])[0][0] as { where: { id: string } };
		expect(args.where.id).toBe("g1");
	});
});
